...
for frame in range(scene.frame_start, 4, 5):
    ...
    for cam_index, cam in enumerate(cameras, 1):
        if cam_index > -1:
            ...
            camera_info = {
                "file_path": os.path.join(
                    "./{data_save_path}/", f"{cam.name:02}_frame_{frame:03}.png"
                ),
        ...
        json_path = os.path.join(frame_directory, f"frame_{frame:03}_{data_save_path}.json")
