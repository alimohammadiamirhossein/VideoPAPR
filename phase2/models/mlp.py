...
def get_mapping_mlp(args, use_amp=True, amp_dtype=torch.float16):
...
class MappingMLP(nn.Module):
    def __init__(self, args, inp_dim=2, out_dim=2, use_amp=True, amp_dtype=torch.float16):
    ...
    def forward(self, x):
        ...
        with autocast(device_type='cuda' if torch.cuda.is_available() else 'cpu', dtype=self.amp_dtype, enabled=self.use_amp):
            out = self.model(x)
            return out
