...
def load_objaverse_model(ckpt_root):
    ...
    ckpt_fname = './105000.ckpt'
    cfg_fname = './configs/sd-objaverse-finetune-c_concat-256.yaml'
    H, W = 256, 256
...
class SD(BaseConf):
    ...
    precision:      str = 'autocast' if device == 'cuda' else 'full'
...
class StableDiffusion(ScoreAdapter):
    def __init__(self, variant, v2_highres, prompt, scale, precision = 'autocast' if device == 'cuda' else 'full', im_path=None):
    ...
    def denoise(self, xs, σ, **model_kwargs):
        with self.precision_scope(device_str):
        ...
        if conditioning_key == 'hybrid':
            ...
            with self.precision_scope(device_str):
                with self.model.ema_scope():
                    ...
                    #print('c_crossattn.shape: ', cond['c_crossattn'][0].shape)
