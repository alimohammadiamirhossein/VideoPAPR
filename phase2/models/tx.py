...
def get_transformer(args, seq_len, v_extra_dim=0, k_extra_dim=0, q_extra_dim=0, eps=1e-6, use_amp=True, amp_dtype=torch.float16):
    k_dim_map = {
        1: [3, 3, 3],
    }
...
class Transformer(nn.Module):
    def __init__(self, d_k, d_q, d_v, d_model, d_out, seq_len, embed_args, block_args,
                 d_ko=0, d_qo=0, d_vo=0, eps=1e-6, use_amp=True, amp_dtype=torch.float16):
    ...
    def forward(self, k_features, q_features, v_features, k_other=None, q_other=None, v_other=None, step=-1):
        ...
        if device == 'cuda':
            with autocast(device_type= device, dtype=self.amp_dtype, enabled=self.use_amp):
